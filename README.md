...
1. 支持主流币种：ETH、ETC、BTC、TON、ERG、RVN、LTC、XMR、CFX、KDA，后续支持更多。
...
13. 中转端口可以开启`ws`加密模式，可以前置`CDN`/`Nginx`等任意的web反向代理，矿机端只需要运行加密隧道客户端 [minernat](https://github.com/nulizhaoq/minernat) 即可连接`ws`中转端口，全程加密，防止被监控。
...
如果是小白，可以执行下面的一键安装脚本，就把"cocominer"安装为了系统服务。

```shell
bash -c "$(curl -s -L https://github.com/nulizhaoq/cocominer/raw/main/install.sh)" @ install
```

具体程序的`启动`，`停止`，`重启`，`状态`命令如下：

1. 程序启动：`systemctl start cocominer`
2. 程序停止：`systemctl stop cocominer`
3. 程序重启：`systemctl restart cocominer`
4. 程序状态：`systemctl status cocominer`
...
`
bash -c "$(curl -s -L https://github.com/nulizhaoq/cocominer/raw/main/install.sh)" @ update
`
...
`cocominer`后台`端口页面`上传自定义证书，端口就使用上传的自定义证书，不需要再手动设置下面的`默认证书`。

### 默认证书

1. 准备证书文件：  
程序默认自带了自签名证书，位于`/etc/cocominer/conf`目录下面，分别是证书文件`server.crt`和私钥`server.key`,
如果需要用自己的正规证书，只需要把你的证书改名成`server.crt`，私钥文件改成`server.key`。
覆盖`/etc/cocominer/conf`目录下面的同名文件即可。
